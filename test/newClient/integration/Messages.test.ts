...
import {InboundMessages, OutboundMessages} from "../../../src/client/models";
...
describe('Client - Message Statistics', function() {
    ...
    it('getOutboundMessages', async() => {
        const messages: OutboundMessages = await client.getOutboundMessages({count:1});
    ...
    it('getInboundMessages', async() => {
        const messages: InboundMessages = await client.getInboundMessages({count:1});
        expect(messages.TotalCount).to.be.gte(0);
